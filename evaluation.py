...
def compute_reasr(responses, targets, num_prompts=None, num_expected_pred=None):
    if num_expected_pred is None:
    ...
    for target_idx in range(len(targets)):
        ...
        max_asr = max(asrs)
        argmax_asr = np.argmax(np.array(asrs))
        # target_asr.append(max_asr)
        target_asr.append([max_asr, argmax_asr])
        all_asrs[target] = asrs
    
    # Step 3: Calculate the average ASR across all targets
    reasr = sum(np.array(target_asr)[:,0]) / len(targets)
    target_asr.append([reasr, -1])
    return target_asr, all_asrs
...
def get_generation(prediction_dict, dataset, model, tokenizer):
    ...
    text_responses = code_generate(prompts, model=model, tokenizer=tokenizer, batch_size=64)
    assert len(prompts) == len(text_responses), "The number of generations does not match the number of triggers."

    return text_responses

def code_generate(prompt, batch_size=32, model=None, tokenizer=None):
